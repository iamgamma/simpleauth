...
import com.gamma.simpleauth.network.ModPacketHandler;
import com.gamma.simpleauth.network.packets.LoginSuccessPacket;
import com.gamma.simpleauth.network.packets.OpenLoginScreenPacket;
import com.gamma.simpleauth.network.packets.OpenRegisterScreenPacket;
...
public class SimpleAuth {
    ...
    public void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        ServerBossEvent bossBar = new ServerBossEvent((Component)Component
                .literal(colorize(getLocalizedMessage("login_bossbar", new Object[] {
                        Integer.valueOf(ConfigManager.getAuthCountdown()) }), "gold")),
        ...
        if (ModPacketHandler.clientHasMod(player)) {
            ...
            player.sendSystemMessage((Component)Component
                    .literal(colorize(getLocalizedMessage("welcome_back", new Object[0]), "green")));
        } else {
            player.sendSystemMessage((Component)Component
                    .literal(colorize(getLocalizedMessage("welcome_register", new Object[0]), "gold")));
        }
    }
    ...
    public void onChat(ServerChatEvent event) {
        ...
        if (!((Boolean)this.playerSessionStatus
                .getOrDefault(player.getUUID(), Boolean.valueOf(false))).booleanValue()) {
            event.setCanceled(true);
        }
    }
    ...
    public class GlobalAuthListener
    {
        ...
        public void onPlayerTick(TickEvent.PlayerTickEvent event) {
            ...
            if (player1 instanceof ServerPlayer) { player = (ServerPlayer)player1; }
            else { return; }
            UUID id = player.getUUID();
            if (((Boolean)SimpleAuth.this.playerSessionStatus
                    .getOrDefault(id, Boolean.valueOf(false))).booleanValue()) {
                return;
            }
            ...
            if (timeLeft > 0) {
                ...
                if (bossBar != null) {
                    ...
                    bossBar.setName((Component)Component.literal(
                            SimpleAuth.colorize(SimpleAuth.this.getLocalizedMessage("login_bossbar"
                                    , new Object[] { Integer.valueOf(timeLeft) }), "gold")));
                }
            } else {

                player.connection.disconnect((Component)Component.literal(
                        SimpleAuth.colorize(SimpleAuth.this.getLocalizedMessage("login_timeout",
                                new Object[0]), "red")));
