...
public class ConfigManager {
    ...
    private static void loadConfigFile() {
        try (Reader reader = new InputStreamReader(new FileInputStream(CONFIG_FILE), StandardCharsets.UTF_8)) {
            ...
            cancelInvulnerabilityOnWalk = config.has("cancelInvulnerabilityOnWalk")
                    ? config.get("cancelInvulnerabilityOnWalk").getAsBoolean()
                    : false;
            kickOnWrongPassword = config.has("kickOnWrongPassword")
                    ? config.get("kickOnWrongPassword").getAsBoolean()
                    : false;
