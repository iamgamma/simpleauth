 ...
 import com.gamma.simpleauth.AuthResult;
 import com.gamma.simpleauth.ConfigManager;
 import com.gamma.simpleauth.DatabaseManager;
 import com.gamma.simpleauth.SimpleAuth;
 ...
 public class PasswordSubmitPacket
 {
   ...
   public static void handle(PasswordSubmitPacket packet, Supplier<NetworkEvent.Context> ctx) {
     ...
     ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> {
           SimpleAuth mod = ModList.get().getModObjectById("simpleauth")
                   .filter(obj -> obj instanceof SimpleAuth).map(obj -> (SimpleAuth) obj).orElse(null);
           if (mod == null) {
             return;
           }
           if (((Boolean)mod.getPlayerSessionStatus().getOrDefault(player.getUUID(), Boolean.valueOf(false)))
                   .booleanValue()) {
             return;
           }
           ...
           if (packet.mode == Mode.REGISTER) {
             ...
             if (DatabaseManager.registerUser(username, packet.password)) {
               ...
               player.sendSystemMessage((Component)Component
                       .literal(SimpleAuth.colorize(mod.getLocalizedMessage("registration_successful",
                               new Object[0]), "green")));
             } else {
               player.sendSystemMessage((Component)Component
             ...
             switch (result) {
               case SUCCESS:
                 mod.completeLogin(player);
                 player.sendSystemMessage((Component)Component
                         .literal(SimpleAuth.colorize(mod.getLocalizedMessage("login_successful",
                                 new Object[0]), "green")));
                 return;

               case RATE_LIMITED:
                 player.connection.disconnect((Component)Component
             ...
             if (ConfigManager.isKickOnWrongPassword()) {
               player.connection.disconnect((Component)Component
                       .literal(SimpleAuth.colorize(mod.getLocalizedMessage("kicked_wrong_password"
                               , new Object[0]), "red")));
             } else {
               ModPacketHandler.sendToPlayer(player,
                       new LoginFailedPacket(LoginFailedPacket.Reason.WRONG_PASSWORD, mod.getRemainingCountdown(player)));
             } 
           } 
         });
     ((NetworkEvent.Context)ctx.get()).setPacketHandled(true);
